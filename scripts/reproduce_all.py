...
figures. Running this script rebuilds the archived Fig. 3 diagnostic and Fig. 5
into ``reproduced/figures`` as a fast review-time reproducibility check. The
GridPACK voltage-control Fig. 4 is generated by ``scripts/build_dc_backbone_v3.py``
from the archived event-sweep outputs; supplemental dynamic screening grids are
regenerated with ``scripts/dynamic_robustness_sweep.py``.
"""
import os
import tempfile
...
def figure3():
    ...
    ax.set_title("b  THD screening", loc="left", fontsize=11, weight="bold")
    ...
    ax.set_title("c  Harmonic spectrum", loc="left", fontsize=11, weight="bold")
...
def figure5():
    ...
    ax.set_xlim(0, 5.25)
    ax.set_ylim(0, 37.5)
    ax.set_title("b  Voltage envelope", loc="left", fontsize=10, weight="bold")
...
def main():
    figure3()
    figure5()
    print(f"Reproduced the archived OpenDSS Fig. 3 diagnostic and Fig. 5 under {OUT}")
