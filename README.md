...
transparent Python models. Fig. 3 is generated from direct OpenDSSDirect.py
harmonic solves. OpenDSS circuit files and the run log are included under
`opendss/`.
...
This repository includes `scripts/run_true_opendss.py`, `opendss/true_opendss_harmonic_network_v3.dss`, and the resulting `data/true_opendss_*` CSV files. The manuscript Fig. 3 uses the direct OpenDSSDirect.py run log in `opendss/true_opendss_run_log_v3.json`.
